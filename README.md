...
| `OpenTopographySource` | **Global fallback** — worldwide SRTM/Copernicus DEMs (~30 m) via the OpenTopography API (free key). |
...
- [`examples/southtyrol_dgm.py`](examples/southtyrol_dgm.py) — the Schlern / Seiser Alm
  (Dolomites) **straight from South Tyrol's WCS** (no local file; needs network).
- [`examples/opentopography_global.py`](examples/opentopography_global.py) — Mont Blanc via the
  **global OpenTopography fallback** (~30 m; needs a free API key in `$OPENTOPOGRAPHY_API_KEY`).
